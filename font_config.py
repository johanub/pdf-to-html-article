...
def font_parser(root):
    ...
    for child in root.iter('fontspec'):
        ...
        if fonts[child.attrib['id']]['size'] >= 25:
            fonts[child.attrib['id']]['tag'] = '<h1 style="color: {}">'.format(fonts[child.attrib['id']]['color'])
            fonts[child.attrib['id']]['endtag'] = '</h1>'
        elif fonts[child.attrib['id']]['size'] >= 24:
            fonts[child.attrib['id']]['tag'] = '<h2 style="color: {}">'.format(fonts[child.attrib['id']]['color'])
